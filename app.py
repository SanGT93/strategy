...
df = pd.read_csv(r'aggr.csv', parse_dates=['Entry time'])

app = dash.Dash(__name__, external_stylesheets=['https://codepen.io/uditagarwal/pen/oNvwKNP.css', 'https://codepen.io/uditagarwal/pen/YzKbqyV.css'])


def filter_df(df, Exchange, Margin, start_date, end_date):
    dff = df.loc[(df['Exchange'] == Exchange) & (df['Margin'] ==int(Margin)) & (df['Entry time'] >= start_date) & (df['Entry time'] <= end_date)]
    return dff


df['YearMonth'] = df.apply(lambda row: str(row['Entry time'].year)+'-'+str(row['Entry time'].month) if len(
    str(row['Entry time'].month)) == 2 else str(row['Entry time'].year)+'-0'+str(row['Entry time'].month), axis=1)
...
def update_graph(exchange, leverage, start_date, end_date):
    dff=filter_df(df, exchange, leverage, start_date, end_date)    
    return {
        'data': [
            go.Bar(
                x = dff[dff['Trade type'] == 'Short']['Entry time'],
                y= dff['Pnl (incl fees)'],
                name = 'Short'
            ),
            go.Bar(
                x=dff[dff['Trade type'] == 'Long']['Entry time'],
                y=dff['Pnl (incl fees)'],
                name = 'Long'
            )
        ],
        'layout': {
            'title': 'Bar chart Entry time vs Pnl',
            'height' : 450
        }
    }
...
def update_balance(exchange, leverage, start_date, end_date):
    ...
    return {
        ...
        'layout': {
            'title': 'Bar chart Entry time vs Balance',
            'height': 450
        }
    }
